...
function adminProfileRedirectWithFlash(string $message, string $type): never
{
...
function adminProfileUploadsDirectory(): string
{
    return dirname(__DIR__, 2) . '/uploads/profile-pictures';
}

function adminProfilePublicPath(string $fileName): string
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formAction === 'upload_profile_picture') {
        ...
        if (!is_array($upload) || (int) ($upload['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
            adminProfileRedirectWithFlash('Please choose an image file to upload.', 'error');
        }

        $imageInfo = @getimagesize((string) ($upload['tmp_name'] ?? ''));
        if ($imageInfo === false) {
            adminProfileRedirectWithFlash('The selected file is not a valid image.', 'error');
        }
        ...
        if (!isset($allowedExtensions[$mimeType])) {
            adminProfileRedirectWithFlash('Only JPG, PNG, and WEBP profile pictures are allowed.', 'error');
        }

        if ((int) ($upload['size'] ?? 0) > 5 * 1024 * 1024) {
            adminProfileRedirectWithFlash('Profile picture must be 5MB or smaller.', 'error');
        }

        $uploadDirectory = adminProfileUploadsDirectory();
        ...
        $relativePath = adminProfilePublicPath($newFileName);
        $absolutePath = $uploadDirectory . '/' . $newFileName;

        if (!move_uploaded_file((string) $upload['tmp_name'], $absolutePath)) {
            adminProfileRedirectWithFlash('Failed to upload profile picture. Please try again.', 'error');
        }
        ...
        adminProfileRedirectWithFlash('Profile picture updated successfully.', 'success');
    }
    ...
    if ($formAction === 'update_email') {
        ...
        if ($newEmail === '') {
            adminProfileRedirectWithFlash('Please enter a valid email address.', 'error');
        }

        if (!filter_var($newEmail, FILTER_VALIDATE_EMAIL)) {
            adminProfileRedirectWithFlash('The email address format is invalid.', 'error');
        }
        ...
        if ($checkStmt) {
            ...
            if (mysqli_stmt_num_rows($checkStmt) > 0) {
                ...
                adminProfileRedirectWithFlash('This email address is already in use by another account.', 'error');
            }
            mysqli_stmt_close($checkStmt);
        }
        ...
        if ($updateStmt) {
            ...
            if (mysqli_stmt_execute($updateStmt)) {
                ...
                adminProfileRedirectWithFlash('Email address updated successfully.', 'success');
            }
            mysqli_stmt_close($updateStmt);
        }

        adminProfileRedirectWithFlash('Failed to update email address. Please try again.', 'error');
    }
    ...
    if ($formAction === 'change_password') {
        ...
        if ($currentPassword === '' || $newPassword === '' || $confirmPassword === '') {
            adminProfileRedirectWithFlash('Please complete all password fields.', 'error');
        }

        if (strlen($newPassword) < 8) {
            adminProfileRedirectWithFlash('New password must be at least 8 characters long.', 'error');
        }

        if ($newPassword !== $confirmPassword) {
            adminProfileRedirectWithFlash('New password and confirmation do not match.', 'error');
        }
        ...
        if ($storedPasswordHash === '' || !password_verify($currentPassword, $storedPasswordHash)) {
            adminProfileRedirectWithFlash('Current password is incorrect.', 'error');
        }
        ...
        adminProfileRedirectWithFlash('Password updated successfully.', 'success');
    }
}
