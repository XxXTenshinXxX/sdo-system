...
// Theme Configuration (User 3 - Blue/Admin)
$themeColor = '#1d4ed8';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formAction === 'update_email') {
        ...
        if ($newEmail === '') {
            user3ProfileRedirectWithFlash('Please enter a valid email address.', 'error');
        }

        if (!filter_var($newEmail, FILTER_VALIDATE_EMAIL)) {
            user3ProfileRedirectWithFlash('The email address format is invalid.', 'error');
        }
        ...
        if ($checkStmt) {
            ...
            if (mysqli_stmt_num_rows($checkStmt) > 0) {
                ...
                user3ProfileRedirectWithFlash('This email address is already in use by another account.', 'error');
            }
            mysqli_stmt_close($checkStmt);
        }
        ...
        if ($updateStmt) {
            ...
            if (mysqli_stmt_execute($updateStmt)) {
                ...
                user3ProfileRedirectWithFlash('Email address updated successfully.', 'success');
            }
            mysqli_stmt_close($updateStmt);
        }

        user3ProfileRedirectWithFlash('Failed to update email address. Please try again.', 'error');
    }
