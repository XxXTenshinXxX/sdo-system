...
// Theme Configuration (User 1 - Green/School)
$themeColor = '#059669';
$themeColorLight = '#d1fae5';
$themeGradient = 'linear-gradient(135deg, #059669, #10b981)';
$themeSurface = 'radial-gradient(circle at top right, rgba(16, 185, 129, 0.08), transparent 34%)';

$pageTitle = 'User 1 Profile';
$activePage = '';

function user1ProfileRedirectWithFlash(string $message, string $type): never
{
...
function user1ProfileUploadsDirectory(): string
{
    return dirname(__DIR__, 2) . '/uploads/profile-pictures';
}

function user1ProfilePublicPath(string $fileName): string
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formAction === 'upload_profile_picture') {
        ...
        if (!is_array($upload) || (int) ($upload['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
            user1ProfileRedirectWithFlash('Please choose an image file to upload.', 'error');
        }

        $imageInfo = @getimagesize((string) ($upload['tmp_name'] ?? ''));
        if ($imageInfo === false) {
            user1ProfileRedirectWithFlash('The selected file is not a valid image.', 'error');
        }
        ...
        if (!isset($allowedExtensions[$mimeType])) {
            user1ProfileRedirectWithFlash('Only JPG, PNG, and WEBP profile pictures are allowed.', 'error');
        }

        if ((int) ($upload['size'] ?? 0) > 5 * 1024 * 1024) {
            user1ProfileRedirectWithFlash('Profile picture must be 5MB or smaller.', 'error');
        }

        $uploadDirectory = user1ProfileUploadsDirectory();
        ...
        $relativePath = user1ProfilePublicPath($newFileName);
        $absolutePath = $uploadDirectory . '/' . $newFileName;

        if (!move_uploaded_file((string) $upload['tmp_name'], $absolutePath)) {
            user1ProfileRedirectWithFlash('Failed to upload profile picture. Please try again.', 'error');
        }
    ...
    if ($formAction === 'change_password') {
        ...
        if ($currentPassword === '' || $newPassword === '' || $confirmPassword === '') {
            user1ProfileRedirectWithFlash('Please complete all password fields.', 'error');
        }

        if (strlen($newPassword) < 8) {
            user1ProfileRedirectWithFlash('New password must be at least 8 characters long.', 'error');
        }

        if ($newPassword !== $confirmPassword) {
            user1ProfileRedirectWithFlash('New password and confirmation do not match.', 'error');
        }
        ...
        if ($storedPasswordHash === '' || !password_verify($currentPassword, $storedPasswordHash)) {
            user1ProfileRedirectWithFlash('Current password is incorrect.', 'error');
        }
        ...
        user1ProfileRedirectWithFlash('Password updated successfully.', 'success');
    }
}
...
<body>
    ...
    <style>
        ...
        .profile-avatar-main {
            ...
            height: 120px;
            border-radius: 50%;
            background: linear-gradient(135deg, <?= $themeColorLight ?>, #f0fdf4);
            color: <?= $themeColor ?>;
            display: flex;
            align-items: center;
